...
- Git操作はこのディレクトリ内で行う。
- 新しいディレクトリや生成物を作る作業では、実装より先に `.gitignore` を確認・整備し、ソースコード用ディレクトリが除外されない規則にする。
- コミットするときは個別ファイルの `git add` を禁止し、必ずリポジトリ直下で `git add .` を使う。
- `git add .` の後に `git status --short` と `git diff --cached --name-status` を確認し、必要な全ファイルがステージ済みであることを確認してからコミットする。
- `output/` のようにソースコード内の同名ディレクトリにも一致する除外規則を使わない。ルートの生成物だけを除外する場合は `/output/` のように先頭 `/` を付ける。
- コミットメッセージは日本語にする。
- すべての読み書きはUTF-8で扱う。
