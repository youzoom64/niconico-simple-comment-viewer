...
from app.services.auto_profile.persona_summary import (
    ...
    load_persona_memo,
    parse_persona_summary_response,
    persona_memo_path,
    save_persona_memo,
)
...
class PersonaSummaryTests(unittest.TestCase):
    def test_build_request_uses_comment_text_only(self) -> None:
        ...
        self.assertIn("コメント本文:", request.prompt)
        self.assertIn("- そうだねぇ、一つにのめり込まずに多面的に見るべき", request.prompt)
        self.assertIn("- 460 煽ったらダメやでｗ", request.prompt)
        self.assertNotIn("冷静な助言役", request.prompt)
        self.assertIn("返答は要約本文だけ", request.prompt)
        self.assertIn("JSON、見出し、箇条書き", request.prompt)
        self.assertNotIn("返答はJSONだけ", request.prompt)
    ...
    def test_parse_persona_summary_response_returns_plain_text_summary(self) -> None:
        plan = parse_persona_summary_response(
            """
            冷静に距離を取りつつ助言する常連。軽い笑いを交えながら現実的に話す。
            """
        )

        self.assertEqual("冷静に距離を取りつつ助言する常連。軽い笑いを交えながら現実的に話す。", plan.persona_summary)
        self.assertEqual("", plan.display_name)
